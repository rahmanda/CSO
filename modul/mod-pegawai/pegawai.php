...
//Query database
$query = pg_query("SELECT idpeg, namadep||' '||namabel as nama, tgllahir, JK, status, idshift from pegawai");
...
if(empty($_GET['act'])) {
			...
			<table id="datatables" class="display">
				...
				while ($r = pg_fetch_array($query)) {
					echo "<tr>
						...
						<td>$r[status]</td>
						<td>$r[idshift]</td>
						<td><a href=\"?module=pegawai&act=edit&idpeg=$r[idpeg]\"><img src=\"img/edit.png\" class=\"edit\" alt=\"edit\"></a>
						&nbsp;&nbsp;<a href='$direct?module=pegawai&act=hapus&id=$r[idpeg]'><img src=\"img/hapus.png\" alt=\"hapus\"></a></td>
						</tr>";
				}                    
				echo '
				</tbody>
	...
	while($r = pg_fetch_array($query)){
		...
		echo	'</select>
				...
				<label for="status">Status</label>
				<div class="status">';
				if(strcmp($fetch['status'],"Cuti")==true) {
				echo '
